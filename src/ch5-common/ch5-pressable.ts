...
export class Ch5Pressable {
        ...
        public getTouchFromTouchList(touchEvent: TouchEvent): Touch | null {
            if (touchEvent.changedTouches !== undefined) {
                ...
                for (let i = 0; i < touchEvent.changedTouches.length; i++) {
                    if (
                        touchEvent.changedTouches[i].identifier ===
                        this.touchPointId
                    ) {
    ...
    private get pressDelayMoveDistance(): number {
        if (
            this._options !== null &&
            !_.isNil(this._options.pressDelayDistance)
        ) {
    ...
    private get clickDelayMoveDistance(): number {
        if (
            this.options !== null &&
            !_.isNil(this.options.clickDelayDistance)
        ) {
    ...
    private _removeEvents() {
        ...
        this._ch5Component.removeEventListener(
            "mouseleave",
            this._onMouseLeave
        );
    ...
    private _subscribeToGestureableProp() {
        if (this._gestureableSubscription === null) {
            this._gestureableSubscription =
                this._ch5Component.observableGestureableProperty.subscribe(
                    (value: boolean) => {
                        if (value) {
    ...
    private _onMouseDown(inEvent: Event): void {
        ...
        if (this._fingerState.mode === Ch5PressableFingerStateMode.Idle) {
            ...
            this._fingerState.touchHoldTimer = window.setTimeout(
                this._onTouchHoldTimer,
                this.pressDelayTime
            );
    ...
    private _onMouseMove(inEvent: Event): void {
        ...
        if (this._fingerState.mode === Ch5PressableFingerStateMode.Start) {
            ...
            if (mouseEvent !== null) {
                const xMoveDistance =
                    mouseEvent.clientX - this._fingerState.touchStartLocationX;
                const yMoveDistance =
                    mouseEvent.clientY - this._fingerState.touchStartLocationY;
                const distanceMoved = Math.sqrt(
                    xMoveDistance ** 2 + yMoveDistance ** 2
                );
                this._ch5Component.info(
                    `DELETE ME Ch5Pressable.onMouseMove() , ${mouseEvent.clientX}, ${mouseEvent.clientY}, ${distanceMoved}`
                );
                if (distanceMoved > this.clickDelayMoveDistance) {
                    this._ch5Component.info(
                        `Ch5Pressable.onMouseMove() cancelling press, ${mouseEvent.clientX}, ${mouseEvent.clientY}, ${distanceMoved}`
                    );
                    this._touchStart = false;
    ...
    private _onMouseUp(inEvent: Event): void {
        ...
        if (mouseEvent !== null) {
            ...
            if (
                this._fingerState.mode ===
                Ch5PressableFingerStateMode.FingerDown
            ) {
    ...
    private _onTouchMove(inEvent: Event): void {
        ...
        if (this._fingerState.mode === Ch5PressableFingerStateMode.Start) {
            const touchEvent: TouchEvent = inEvent as TouchEvent;
            const touch: Touch | null =
                this._fingerState.getTouchFromTouchList(touchEvent);
            if (touch !== null) {
                // this._ch5Component.info(`Ch5Pressable._onTouchMove() , ${touch.clientX}, ${touch.clientY}, ${touch.identifier}`);
                const xMoveDistance =
                    touch.clientX - this._fingerState.touchStartLocationX;
                const yMoveDistance =
                    touch.clientY - this._fingerState.touchStartLocationY;
                const distanceMoved = Math.sqrt(
                    xMoveDistance ** 2 + yMoveDistance ** 2
                );
                this._ch5Component.info(
                    `DELETE ME Ch5Pressable._onTouchMove() , ${touch.clientX}, ${touch.clientY}, ${touch.identifier}, ${distanceMoved}`
                );
                if (distanceMoved > this.pressDelayMoveDistance) {
                    this._ch5Component.info(
                        `Ch5Pressable._onTouchMove() cancelling press, ${touch.clientX}, ${touch.clientY}, ${touch.identifier}, ${distanceMoved}`
                    );
                    this._touchStart = false;
                    this._fingerState.reset();
                    if (isIosDevice()) {
    ...
    private _onHold() {
        this._ch5Component.info(
            `Ch5Pressable._onHold() alreadyPressed:${this._pressed}`
        );
        if (!this._pressed) {
    ...
    private _onRelease() {
        this._ch5Component.info(
            `Ch5Pressable._onRelease() alreadyReleased:${this._released}`
        );
        if (!this._released) {
            ...
            // simply eval()uate the on<event> text.
            // This is consistent with how standard events are processed, example <div onclick="">
            const onReleaseAttrib =
                this._ch5Component.getAttribute("onrelease");
